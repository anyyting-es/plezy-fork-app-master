...
static void mpv_plugin_handle_method_call(FlMethodChannel* channel, FlMethodCall* method_call, gpointer user_data) {
  ...
  } else if (strcmp(method, "getGpuName") == 0) {
    std::string gpu = self->player ? self->player->GetRendererName() : "";
    response = FL_METHOD_RESPONSE(fl_method_success_response_new(fl_value_new_string(gpu.c_str())));
  } else {
