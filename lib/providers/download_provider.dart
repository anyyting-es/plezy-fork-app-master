...
class DownloadProvider extends ChangeNotifier with DisposableChangeNotifierMixin {
  ...
  final List<MediaItem> _completedTorrents = [];
  ...
  DownloadProvider({required this._downloadManager, required this._database})
    ...
    // Listen to completed torrents from Go backend
    _torrentSubscription = TorrentEngineService.instance.watchTorrents().listen((torrents) async {
      final completedItems = <MediaItem>[];
      await TorrentMetadataService.instance.initialize();
      for (final t in torrents) {
        final isCompleted = t.progress >= 100 || (t.size > 0 && t.downloaded >= t.size);
        if (isCompleted) {
          final meta = TorrentMetadataService.instance.getMetadata(t.infoHash);
          if (meta != null) {
            completedItems.add(meta);

            final serverId = meta.serverId ?? 'torrent';
            final ratingKey = meta.id;
            final globalKey = buildGlobalKey(ServerId(serverId), ratingKey);
            final exists = await _database.getDownloadedMedia(globalKey);
            if (exists == null) {
              unawaited(_moveTorrentFiles(t.infoHash, t.name, t.files, meta));
            }
          }
        }
      }
      _completedTorrents.clear();
      _completedTorrents.addAll(completedItems);
      safeNotifyListeners();
    });
  }
  ...
  List<MediaItem> get downloadedShows {
    ...
    // Add completed torrent shows
    for (final meta in _completedTorrents) {
      if (meta.isShow) {
        shows[meta.id] = meta;
      } else if (meta.isSeason) {
        final showId = meta.parentId ?? 'show_${meta.id}';
        shows[showId] = MediaItem(
          id: showId,
          backend: meta.backend,
          kind: MediaKind.show,
          title: meta.grandparentTitle ?? meta.title ?? t.common.unknown,
          thumbPath: meta.grandparentThumbPath ?? meta.thumbPath,
          artPath: meta.grandparentArtPath ?? meta.artPath,
          serverId: meta.serverId,
          raw: meta.raw,
        );
      } else if (meta.isEpisode) {
        final showId = meta.grandparentId ?? 'show_${meta.id}';
        shows[showId] = MediaItem(
          id: showId,
          backend: meta.backend,
          kind: MediaKind.show,
  ...
  List<MediaItem> get downloadedMovies {
    ...
    movies.addAll(_metadata.entries
        .where((entry) {
    ...
    // Add completed torrent movies
    movies.addAll(_completedTorrents.where((meta) => meta.isMovie));
  ...
  List<MediaItem> getDownloadedEpisodesForShow(String showRatingKey) {
    ...
    episodes.addAll(_metadata.entries
        .where((entry) {
          if (!_ownsDownloadKey(entry.key)) return false;
          final progress = _downloads[entry.key];
          final meta = entry.value;
    ...
    // Add completed torrent episodes
    episodes.addAll(_completedTorrents.where((meta) =>
        meta.isEpisode &&
        (meta.grandparentId == showRatingKey || meta.parentId == showRatingKey)));
  ...
  Future<void> _moveTorrentFiles(String infoHash, String torrentName, List<TorrentFile> files, MediaItem metadata) async {
    try {
      // 1. Get download dir (cache) and downloads dir (organized)
      final downloadDir = await TorrentEngineService.instance.getDownloadDirectory();
      final downloadsDir = await TorrentEngineService.instance.getDownloadsDirectory();

      // 2. Determine target path
      final subfolder = metadata.isMovie ? 'Movies' : 'Shows';
      final cleanTitle = _sanitizeFolderName(metadata.title ?? torrentName);
      final targetBaseDir = Directory('$downloadsDir/downloads/$subfolder/$cleanTitle');

      if (!await targetBaseDir.exists()) {
        await targetBaseDir.create(recursive: true);
      }
      ...
      for (final f in files) {
        final srcFile = File('$downloadDir/${f.path}');
        if (await srcFile.exists()) {
          final fileName = path.basename(f.path);
          final destFile = File('${targetBaseDir.path}/$fileName');

          appLogger.i('[torrent-move] Moving $srcFile to $destFile...');
          try {
            await srcFile.rename(destFile.path);
          } catch (e) {
            appLogger.d('[torrent-move] Rename failed ($e), falling back to copy');
            try {
              await srcFile.copy(destFile.path);
              await srcFile.delete();
            } catch (e2) {
              appLogger.e('[torrent-move] Copy failed: $e2');
              continue;
            }
          }

          final ext = path.extension(f.path).toLowerCase();
          if (videoExtensions.contains(ext) && f.size > maxVideoSize) {
            maxVideoSize = f.size;
            mainVideoPath = destFile.path;
          }
        }
      }

      if (mainVideoPath == null) {
        appLogger.w('[torrent-move] No main video file found among moved files!');
        return;
      }

      // 3. Register in database
      final serverId = metadata.serverId ?? 'torrent';
      final ratingKey = metadata.id;
      final globalKey = buildGlobalKey(ServerId(serverId), ratingKey);

      appLogger.i('[torrent-move] Registering $globalKey in database with path $mainVideoPath');

      await _database.insertDownload(
        serverId: ServerId(serverId),
        ratingKey: ratingKey,
        globalKey: globalKey,
        type: metadata.isMovie ? 'movie' : 'episode',
        status: DownloadStatus.completed.index,
        parentRatingKey: metadata.parentId,
        grandparentRatingKey: metadata.grandparentId,
      );

      await _database.updateVideoFilePath(globalKey, mainVideoPath);

      // 4. Force reload downloads in provider so the UI updates
      await _loadPersistedDownloads();

      // 5. Remove torrent from engine so it doesn't seed/clutter
      await TorrentEngineService.instance.removeTorrent(infoHash, deleteFiles: false);
    } catch (e) {
      appLogger.e('[torrent-move] Error moving torrent files', error: e);
    }
  }
}
