...
TrackPreferencePersister _plexTrackPersister(dynamic Function() resolve) {
...
class VideoPlayerScreen extends StatefulWidget {
  ...
  /// Local or direct HTTP URL to play immediately, bypassing server resolution.
...
class VideoPlayerScreenState extends State<VideoPlayerScreen> with WidgetsBindingObserver, MountedSetStateMixin {
  ...
  Future<void> _initializePlayer() async {
    ...
    try {
      ...
      if (widget.directStreamUrl != null && mounted) {
        _serverSupportsTranscoding = false;
        _selectedQualityPreset = TranscodeQualityPreset.original;
        
        final mockResult = PlaybackInitializationResult(
          availableVersions: const [],
          videoUrl: widget.directStreamUrl,
          mediaInfo: null,
        ...
        _playbackDataFuture = Future.value(PlaybackContext(
      ...
      } else if (!widget.isLive && !_offlineLibraryMode && mounted) {
