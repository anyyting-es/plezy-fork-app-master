...
class _MediaDetailScreenState extends State<MediaDetailScreen>
  ...
  Future<void> _loadSeasons() async {
      ...
      if (_metadata.isAnilist) {
        try {
          ...
          if (_metadata.id.startsWith('anime_')) {
            final animeId = _metadata.id.substring('anime_'.length);
            animeSeasons = [
              MediaItem.anilist(
                id: 'anime_season_$animeId',
                kind: MediaKind.season,
                title: 'Season 1',
                index: 1,
                parentId: _metadata.id,
                thumbPath: _metadata.thumbPath,
                artPath: _metadata.artPath,
              )
            ];
          } else {
            animeSeasons = const [];
          }
          _updateSeasonTabFocusNodes(animeSeasons.length);
          if (!mounted) return;
          setState(() {
            _seasons = animeSeasons;
            _isLoadingSeasons = false;
            _hasLoadedSeasons = true;
            _showEpisodesDirectly = animeSeasons.length <= 1;
    ...
    try {
      ...
      final prefsFuture = Future.value(<String, dynamic>{});

      final results = await Future.wait<dynamic>(<Future<dynamic>>[seasonsFuture, prefsFuture]);
        ...
        if (mediaClient == null) {
          if (_metadata.isAnilist) {
            ...
            if (_metadata.id.startsWith('anime_')) {
              ...
              final episodesList = await AniZipService.getEpisodes(animeId);
              animeEpisodes = episodesList.map((ep) => MediaItem.anilist(
  ...
  Future<void> _loadRelatedHubs() async {
    ...
    try {
      ...
      if (_metadata.isAnilist || _metadata.isTmdb) {
        ...
        final client = serverId == null ? null : context.tryGetMediaClientForServer(ServerId(serverId));
        if (client != null) {
          relatedHubs = await client.fetchRelatedHubs(_metadata.id);
        }
      }
