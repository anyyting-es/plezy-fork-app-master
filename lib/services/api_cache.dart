...
class DefaultApiCache extends ApiCache {
  DefaultApiCache(super.db);

  @override
  Future<MediaItem?> getMetadata(ServerId serverId, String itemId) async {
    final key = '$serverId:$itemId';
    final row = await (database.select(database.apiCache)
  ...
  Future<void> pinForOffline(ServerId serverId, String itemId) async {
    final key = '$serverId:$itemId';
    await (database.update(database.apiCache)
          ..where((t) => t.cacheKey.equals(key)))
  ...
  Future<void> deleteForItem(ServerId serverId, String itemId) async {
    final key = '$serverId:$itemId';
    await (database.delete(database.apiCache)
          ..where((t) => t.cacheKey.equals(key)))
        .go();
  }
  ...
  Future<void> applyWatchState({
    ...
    // Basic neutral watch state updates on cached JSON
    final key = '$serverId:$itemId';
    final row = await (database.select(database.apiCache)
          ..where((t) => t.cacheKey.equals(key)))
        .getSingleOrNull();
    ...
    try {
      final map = json.decode(row.data) as Map<String, dynamic>;
      // Mutate neutral fields (e.g. viewCount/playCount/played)
      map['viewCount'] = isWatched ? 1 : 0;
      map['played'] = isWatched;
      if (map['userData'] != null) {
        final userData = map['userData'] as Map<String, dynamic>;
        userData['played'] = isWatched;
        userData['playCount'] = isWatched ? 1 : 0;
      }
      final encoded = json.encode(map);
      await (database.update(database.apiCache)
            ..where((t) => t.cacheKey.equals(key)))
          .write(ApiCacheCompanion(data: Value(encoded)));
    } catch (_) {}
  }

  @override
  Future<Map<String, MediaItem>> getAllPinnedMetadata() async {
    final rows = await (database.select(database.apiCache)
