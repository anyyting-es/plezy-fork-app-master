...
class OptimizedMediaImage extends StatelessWidget {
  ...
  Widget _buildLocalFileImage(BuildContext context, File file, double effectiveWidth, double effectiveHeight) {
    ...
    return Image.file(
      ...
      width: width,
      height: height,
      // Decode based on width for landscape background art since container is wide;
      // otherwise use height to preserve resolution and aspect ratios.
      cacheWidth: imageType == ImageType.art ? (memWidth > 0 ? memWidth : null) : null,
      cacheHeight: imageType == ImageType.art ? null : (memHeight > 0 ? memHeight : null),
  ...
  Widget _buildCachedImage(BuildContext context, double effectiveWidth, double effectiveHeight) {
    ...
    final resizedProvider = ResizeImage.resizeIfNeeded(
      imageType == ImageType.art ? (memWidth > 0 ? memWidth : null) : null,
      imageType == ImageType.art ? null : (memHeight > 0 ? memHeight : null),
      provider,
    );
