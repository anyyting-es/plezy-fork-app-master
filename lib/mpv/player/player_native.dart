...
class PlayerNative extends PlayerBase {
    ...
    if (media.headers != null && media.headers!.isNotEmpty) {
      ...
      for (final entry in media.headers!.entries) {
        ...
        if (keyLower == 'user-agent') {
          await setProperty('user-agent', entry.value);
        } else if (keyLower == 'referer' || keyLower == 'referrer') {
          await setProperty('referrer', entry.value);
        }
      }
    } else {
      await setProperty('http-header-fields', '');
      await setProperty('user-agent', '');
      await setProperty('referrer', '');
    }
  ...
  Future<String?> getGpuName() async {
    if (disposed) return null;
    try {
      final String? gpu = await invoke<String>('getGpuName');
      return gpu;
    } catch (_) {
      return null;
    }
