...
sealed class MediaItem with _$MediaItem {
  ...
  factory MediaItem({
    ...
    return switch (backend) {
      ...
      MediaBackend.anilist => AnilistMediaItem(
        id: id,
        kind: kind,
        guid: guid,
        title: title,
  ...
  /// Backend-tagged concrete subclass for items sourced from AniList.
  @FreezedUnionValue('anilist')
  @JsonSerializable(includeIfNull: false, explicitToJson: true)
  const factory MediaItem.anilist({
    @JsonKey(readValue: readStringField, defaultValue: '') required String id,
    @JsonKey(fromJson: _mediaKindFromJson, toJson: _mediaKindToJson) required MediaKind kind,
    String? guid,
    String? title,
