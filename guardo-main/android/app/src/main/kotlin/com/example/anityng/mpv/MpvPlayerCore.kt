package com.example.anityng.mpv
...
import com.example.anityng.shared.AudioFocusManager
import com.example.anityng.shared.FlutterOverlayHelper
import com.example.anityng.shared.FrameRateManager
import com.example.anityng.shared.PlayerDelegate
...
class MpvPlayerCore(private val activity: Activity) : SurfaceHolder.Callback {
  ...
  fun initialize(onResult: (Boolean) -> Unit) {
    ...
    try {
      ...
      frameRateManager = FrameRateManager(
        activity = activity,
        handler = handler,
        log = { text: String -> emitLog("info", "framerate", text) }
      )
  ...
  fun setVideoFrameRate(
    ...
    mgr.setVideoFrameRate(fps, videoDurationMs, extraDelayMs) { switched: Boolean ->
